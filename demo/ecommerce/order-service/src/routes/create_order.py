...
def create_order(req: CreateOrderRequest, authorization: str | None = Header(default=None)):
    ...
    try:
    ...
    except payments.PaymentTimeout:
        ...
        raise HTTPException(status.HTTP_504_GATEWAY_TIMEOUT, "payment timed out")
        ...
        raise HTTPException(status.HTTP_502_BAD_GATEWAY, "payment failed")

    # Step 4 — mark PAID.
    _safe_set_status(order_id, "PAID")
    orders_created_total.inc()
    log.info("order created", extra={"order_id": order_id, "user_id": user["id"], "amount": req.amount})
