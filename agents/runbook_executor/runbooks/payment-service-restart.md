...
- name: verify-health
  ...
  namespace: ecommerce
---
# payment-service — restart (generic recovery)
...
payment-service degraded with no identified injected fault.
...
```powershell
kubectl -n ecommerce annotate pod -l app=payment-service drain=true --overwrite
```
...
```powershell
kubectl -n ecommerce rollout restart deploy/payment-service
